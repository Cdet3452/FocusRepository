...
-- FIXES:
--   1. Sets LastServerPurchaseTick on the player when server confirms a purchase
--      so UIController's safe window resets from the server side too
--   2. Removed duplicate AddButtonJuice calls
--   3. Local T variable declared only once (was declared twice causing shadow)
...
local function BuildCard(parent, upgradeId, cfg, isEpic, cardRefsTable)
	...
	local function TryBuy()
		if isEpic then
			...
			player:SetAttribute("LocalAuraSpend", state.cost)
			...
			local wasMaxedLocally = state.maxed
			player:SetAttribute("LocalSpend", state.cost)
	...
	buyButton.MouseButton1Down:Connect(function()
		...
		while holdingBuy and buyGeneration == myGen do
			if not UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) then
				holdingBuy = false; break
			end
			local success = TryBuy()
			if not success then holdingBuy = false; break end
...
-- ─────────────────────────────────────────────────────────────────────────────
-- CARD UPDATE FUNCTIONS  (declared with function so they can call each other)
-- ─────────────────────────────────────────────────────────────────────────────
function UpdateRegularCard(upgradeId)
	...
	if state.level >= state.maxLevel then
		...
		ref.buyButton.TextColor3   = (currentCash < state.cost)
...
function UpdateEpicCard(upgradeId)
	...
	if state.level >= state.maxLevel then
		...
		ref.frame.LayoutOrder      = ref.baseOrder or 0
		ref.buyButton.Text         = "✦ " .. FormatNumber(state.cost)
		ref.buyButton.TextColor3   = (currentAuras < state.cost)
			and Color3.fromRGB(255, 100, 100) or Color3.fromRGB(255, 255, 255)
...
task.delay(1, function() isLoadingData = false end)
...
if UpgradeUpdated then
	UpgradeUpdated.OnClientEvent:Connect(function(info)
		...
		elseif info.type == "purchased" then
			-- ── FIX: stamp the server confirmation tick so UIController's
			--         safe-window resets from both ends ────────────────────
...
if EpicUpgradeUpdated then
	EpicUpgradeUpdated.OnClientEvent:Connect(function(info)
		...
		elseif info.type == "purchased" then
			-- ── FIX: stamp the server confirmation tick ───────────────────
			player:SetAttribute("LastServerPurchaseTick", tick())
