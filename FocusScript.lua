...
-- Focus Script: ClickHandler (Client)
local Players = game:GetService("Players")
local ReplicatedStorage = game:GetService("ReplicatedStorage")
local TweenService = game:GetService("TweenService")
...
local visualStorageQueue = {}
local MAX_VISUAL_AURAS = 750
					...
					for id, data in pairs(cubeDataMap) do
						if data.instance == auraObj and not data.isStored then
							...
							if root then
								...
								root.AssemblyAngularVelocity = Vector3.new(math.random(-10, 10), math.random(-10, 10), math.random(-10, 10))

								task.delay(1.5, function()
									if auraObj and auraObj.Parent then
										...
										table.insert(visualStorageQueue, auraObj)
										if #visualStorageQueue > MAX_VISUAL_AURAS then
											local oldestAura = table.remove(visualStorageQueue, 1)
											if oldestAura and oldestAura.Parent then
												pcall(function() PoolManager.ReturnAura(oldestAura) end)
											end
										end
									end
								end)
...
AreaChanged.OnClientEvent:Connect(function()
	...
	table.clear(visualStorageQueue)
	lastTierIndex = 1
...
-- Focus Script: AuraSpawner (Server)
local Players             = game:GetService("Players")
local ReplicatedStorage   = game:GetService("ReplicatedStorage")
local ServerScriptService = game:GetService("ServerScriptService")
...
Players.PlayerRemoving:Connect(function(p)
	hatchery[p.UserId] = nil
	holdStart[p.UserId] = nil
	...
	while true do
		...
		for _, player in ipairs(Players:GetPlayers()) do
			...
			for cubeId, cube in pairs(runtime.cubes) do
				...
				cube.effectiveElapsed = cube.effectiveElapsed + dt
				...
				while cube.tierIndex < maxTier and cube.tierIndex < #TierConfig.Tiers and upgrades < 5 do
					...
					if math.random() <= bestChance then
						...
						cube.tierIndex = cube.tierIndex + 1
						...
						upgrades = upgrades + 1; mutated = true
