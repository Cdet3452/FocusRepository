...
local function UpdateHabitatBar(actualPending, max)
	-- ✨ SYNC: Read the visual offset so the text counts down cleanly
...
-- Allow text to count down instantly as magnet moves blocks
...
UpdateHUDBridge:Connect(function(stats)
	if stats.passiveInterval ~= nil then currentPassiveInterval = stats.passiveInterval end
		...
		if info.mutationType == "tierUpgrade" then
			...
			if newAura then
				...
				if cubeData.rateLabel and cubeData.rateLabel.Parent then
					...
					if cubeData.isStored then cubeData.rateLabel.Enabled = false end
				end
			...
			if not data.isStored and data.instance and data.instance.Parent then
				...
				if root then
					...
					if currentPos.Y < data.spawnY - 12 then
						...
						data.instance:Destroy(); cubeDataMap[id] = nil
						continue
					end
